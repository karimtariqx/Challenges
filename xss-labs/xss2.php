...
<html>
<head>
    <title>Stored XSS - Medium Difficulty</title>
    <style>
...
<body>
    <h2>Stored XSS Challenge 2: H4CK3RS SL4Y3R</h2>
    <form method="get">
...
if (isset($_POST['clear_comments'])) {
    $_SESSION['comments2'] = [];  // Clear the comments
}
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['comment'])) {
    $comment = $_GET['comment']; // Get the raw comment
    $sanitizedComment = basicSanitization($comment);
    $decodedSanitizedComment = urldecode($sanitizedComment); // Decode the sanitized input
    $_SESSION['comments2'][] = $decodedSanitizedComment;
}
